	...
	for l.Len() != 0 {
		...
		for _, succ := range agTemp.Board.GetNeighbours(agTemp.Position) {
			...
			if !ok {
				...
				newHum := NewHuman(agent.ID, agent.Type, agent.Race, agent.Body, agent.Stats, succ, agent.Target, agent.MovingToTarget, agent.CurrentPath, agent.Board, agent.ComOut, agent.ComIn)
